...
| 실제 DOM (Real DOM)                             | 가상 DOM (Virtual DOM)                                  |
| ----------------------------------------------- | ------------------------------------------------------- |
| 웹페이지의 실제 구조를 나타냅니다.              | 웹페이지의 가상/메모리 상의 표현을 나타냅니다.          |
| DOM 조작이 매우 비용이 많이 듭니다.             | DOM 조작이 매우 쉽습니다.                               |
| HTML을 직접 업데이트할 수 있습니다.             | HTML을 직접 업데이트할 수 없습니다.                     |
| 요소가 업데이트되면 새로운 DOM을 생성합니다.    | 요소가 업데이트되면 JSX를 업데이트합니다.               |
| 특정 노드(HTML 요소)를 직접 타겟할 수 있습니다. | 초당 약 200,000개의 가상 DOM 노드를 생성할 수 있습니다. |
| 애플리케이션의 UI를 나타냅니다.                 | DOM의 가상 표현일 뿐입니다.                             |

### 2.2.3 가상 DOM을 위한 아키텍처, 리액트 파이버

- **리액트 파이버(React Fiber)**
  - 가상 DOM과 렌더링 최적화를 위해 리액트에서 사용하는 아키텍처입니다.
  - React Fiber는 React 16에서 도입된 React의 "Fiber Reconciliation"이라는 알고리즘을 사용하여 가상 DOM을 구현합니다.
  - 이 알고리즘은 이전의 스택 기반 알고리즘 대신에 비동기적으로 작동하며, 컴포넌트의 렌더링 및 업데이트를 더 효율적으로 처리할 수 있도록 설계되었습니다.
  - React Fiber에서는 컴포넌트가 최초로 마운트되고 생성된 이후에는 가능한 한 재사용됩니다. 이는 React의 재렌더링 과정에서 이전에 생성된 컴포넌트 인스턴스를 재사용하여 성능을 최적화하는 방식입니다.

### 2.2.4 파이버와 가상 DOM

- **파이버의 장점**

  - **비동기 렌더링 (Asynchronous Rendering)**: Fiber는 비동기 렌더링을 지원하며, 이는 렌더링 작업을 여러 프레임으로 나눌 수 있게 해줍니다. 이로써 렌더링 작업이 브라우저의 주 스레드를 오랫동안 점유하지 않고 사용자 경험을 개선할 수 있습니다.
  - **취소 가능한 렌더링 (Cancelable Rendering)**: Fiber는 렌더링 작업을 취소할 수 있게 해줍니다. 이는 렌더링 작업이 불필요하게 진행되는 것을 방지하고 주 스레드를 오랫동안 점유하지 않도록 합니다.
  - **우선순위 기반 렌더링 (Priority-Based Rendering)**: Fiber를 사용하면 렌더링 작업의 우선순위를 지정할 수 있습니다. 이렇게 하면 렌더링 작업이 주 스레드를 지나치게 점유하지 않고 사용자 상호작용에 높은 우선순위를 부여할 수 있습니다.
  - **자체 타임 슬라이싱 (Self Time Slicing)**: Fiber는 자체적으로 타임 슬라이싱을 수행하여 렌더링 작업이 주 스레드를 오랫동안 점유하지 않도록 합니다.
  - **코드 분할 (Code Splitting)**: Fiber는 코드 분할을 지원하여 렌더링 작업이 불필요하게 진행되는 것을 방지하고 전체 성능을 향상시킵니다.
  - **렌더링 작업 중단 및 재시작 (Pausing and Resuming Rendering)**: Fiber는 렌더링 작업을 일시 중단하고 재개할 수 있어 불필요한 작업을 방지하고 주 스레드를 오랫동안 점유하지 않도록 합니다.
  - **메모리 절약 (Memory Efficiency):** Fiber는 메모리 효율성을 강조하며, 렌더링 작업이 메모리를 과도하게 소비하지 않고 성능을 향상시킵니다.
  - **텍스트 노드 재사용 (Text Node Reuse)**: Fiber는 텍스트 노드를 재사용하여 불필요한 렌더링 작업을 방지하고 효율성을 개선합니다.
  - **브라우저 작업 예약 (Browser Task Scheduling)**: Fiber는 브라우저 작업을 예약할 수 있어 렌더링 작업이 불필요하게 진행되지 않도록 합니다.

- **파이버의 단점**
  - **메모리 사용량 증가 (Increased Memory Usage)**: Fiber는 주 스레드를 오랫동안 점유하지 않도록 메모리 사용량을 증가시킬 수 있으며 이는 성능 향상을 위한 것입니다.
  - **초기 렌더링 속도 저하 (Initial Rendering Speed Decrease)**: Fiber는 초기 렌더링 속도가 느려질 수 있으며 이는 주 스레드를 지나치게 점유하지 않도록 하는 데 중점을 둔 것입니다.
