...
React 18 변경 사항: `Suspense`의 일관된 적용

**Suspense의 동작 변경**

React 18에서는 `Suspense` 컴포넌트의 동작이 개선되어 이제 항상 일관되게 적용됩니다. 이전에는 컴포넌트가 `suspend` 상태가 되었을 때, React가 불완전한 상태의 컴포넌트를 트리에 추가하거나 `effect`를 발생시킬 수 있었습니다. 하지만 React 18에서는 컴포넌트가 `suspend` 되면, React는 해당 컴포넌트를 트리에 추가하지 않고, 불완전한 상태로 남겨두지 않습니다.

대신, React는 새로운 트리를 완전히 버리고, 비동기 작업이 완료될 때까지 기다립니다. 그 후, 비동기 작업이 끝나면 처음부터 다시 렌더링을 시도합니다. 이 과정은 브라우저를 차단하지 않고 비동기적으로 이루어지므로, 사용자 경험을 저해하지 않으면서 동시에 렌더링을 재시도 할 수 있습니다.

**Suspense와 `Layout Effect`**

React 18에서는 Suspense로 인한 `fallback` 발생 시의 `layout effect` 처리가 개선되었습니다. 트리가 `suspend` 상태가 되어 `fallback`으로 전환될 때, React는 해당 컴포넌트의 `layout effect`를 정리합니다. 그리고 `boundary` 내부의 내용이 다시 표시될 때까지 `layout effect`의 실행을 유보합니다.

이러한 변경은 컴포넌트 라이브러리가 `Suspense`와 함께 사용될 때 발생했던 레이아웃 측정 문제를 해결합니다. 이전에는 `Suspense`로 인해 컴포넌트가 비동기적으로 로드되는 동안 레이아웃 `effect`가 잘못 적용되어 측정 문제가 발생할 수 있었지만, 이제는 해당 문제가 해결되어 더 정확한 레이아웃 측정이 가능해졌습니다.

**[`Suspense`란?]**

`Suspense`는 React에서 비동기 작업을 처리하는 방법 중 하나로, 컴포넌트가 데이터를 기다리는 동안 로딩 상태를 표시하고, 데이터가 준비되면 실제 컴포넌트를 렌더링하는 기능을 말합니다. 이를 통해 비동기 로직과 그에 따른 상태 관리를 컴포넌트 트리 외부로 끌어낼 수 있으며, 데이터 로딩과 같은 비동기 작업을 선언적으로 처리할 수 있습니다.
...
```
이 예에서 `MyDataComponent`는 비동기적으로 데이터를 불러오고 있으며, 데이터가 준비되기 전까지는 `<Suspense>` 컴포넌트 내부의 `fallback`으로 정의된 로딩 인디케이터가 화면에 표시됩니다.

**[주의]**

`Suspense`는 `React 16.6`부터 도입되었으며, `React 18`에서는 `Suspense`의 동작이 개선되어 더욱 일관된 경험을 제공합니다. 그러나 모든 비동기 작업에 `Suspense`를 사용할 수 있는 것은 아니며, 현재는 주로 코드 스플리팅에 사용되고 있습니다. 데이터 패칭 라이브러리와 함께 사용할 때도 있지만, 이 경우 해당 라이브러리가 `Suspense`와 호환되어야 합니다.

### 10.2.7 인터넷 익스플로러 지원 중단에 따른 추가 폴리필 필요

**[새로운 JavaScript 환경과 폴리필 필요성]**

`React 18`은 현대 브라우저의 기능인 `Promise`, `Symbol`, `Object.assign` 등에 의존합니다. 이러한 최신 기능들은 오래된 브라우저에서 기본적으로 제공되지 않습니다. 따라서, 인터넷 익스플로러와 같은 호환되지 않는 브라우저를 지원해야 하는 경우, 애플리케이션에 폴리필을 추가해야 할 필요가 있습니다.

**[폴리필 추가 방법]**

폴리필을 추가하는 일반적인 방법은 `core-js` 또는 `babel-polyfill`과 같은 라이브러리를 사용하는 것입니다. 이러한 라이브러리들은 오래된 브라우저에서도 최신 `JavaScript` 기능을 사용할 수 있게 도와줍니다.

**[예제1: 전체 추가]**

`React` 애플리케이션의 최상단에 다음 코드를 추가하여 전체 폴리필을 적용할 수 있습니다.
...
필요한 특정 기능에 대해서만 폴리필을 적용하고 싶다면, 다음과 같이 해당 기능의 폴리필만 가져올 수 있습니다.
...
이러한 폴리필을 프로젝트에 추가할 때는 애플리케이션의 크기가 증가할 수 있으므로, 실제로 필요한 폴리필만 선택적으로 추가하는 것이 좋습니다. 또한, 오래된 브라우저 지원이 비즈니스 요구사항인지 잘 고려해야 합니다. 가능하다면, 사용자에게 모던 브라우저 사용을 권장하는 것도 한 방법입니다.
